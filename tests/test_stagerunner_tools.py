...
def _ctx(tmp_path: Path, output_format: str = "json", *, args: tuple[str, ...] = ()) -> StageContext:
    return StageContext(
...
def test_ruff_lint_argv_json_flag(tmp_path: Path) -> None:
    assert tools.ruff_lint_argv(_ctx(tmp_path, "json")) == ["ruff", "check", "--output-format", "json", "."]
    assert tools.ruff_lint_argv(_ctx(tmp_path, "text")) == ["ruff", "check", "."]
...
def test_bandit_argv_uses_package_name_and_format(tmp_path: Path) -> None:
    ctx_json = dataclasses.replace(_ctx(tmp_path, "json"), package_name="acme")
    assert tools.bandit_argv(ctx_json) == ["bandit", "-r", "-c", "pyproject.toml", "-f", "json", "src/acme"]
    ctx_text = dataclasses.replace(_ctx(tmp_path, "text"), package_name="acme")
...
def test_bandit_argv_derives_package_name(tmp_path: Path) -> None:
    ...
    assert tools.bandit_argv(_ctx(tmp_path, "text"))[-1] == "src/der_ived"


def test_pyright_argv_json_flag(tmp_path: Path) -> None:
    assert tools.pyright_argv(_ctx(tmp_path, "json")) == ["pyright", "--outputjson"]
    assert tools.pyright_argv(_ctx(tmp_path, "text")) == ["pyright"]
...
def test_pip_audit_argv_with_ignores_and_json(tmp_path: Path) -> None:
    ...
    assert tools.pip_audit_argv(_ctx(tmp_path, "json")) == ["pip-audit", "--ignore-vuln=GHSA-x", "-f", "json"]
    assert tools.pip_audit_argv(_ctx(tmp_path, "text")) == ["pip-audit", "--ignore-vuln=GHSA-x"]


def test_pytest_cov_argv(tmp_path: Path) -> None:
    argv = tools.pytest_cov_argv(_ctx(tmp_path, "json"))
