...
(`make test`, `make push`, `make bump-patch`, ...). The env belongs to that one repo, and is re-resolved on every `make`, so bmk and the project's dependencies are always current. 
