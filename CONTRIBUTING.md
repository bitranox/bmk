...
- `make test` runs Ruff (lint + format check), Pyright, and Pytest with coverage. Tool output defaults to JSON mode, where the stagerunner captures all output and only displays it when a stage fails -- a fully passing run produces zero tool output. Dependencies are checked silently, Makefile version updates are auto-accepted, and pytest uses concise flags (`--tb=short -q --no-header`). Use `bmk test --human` or `BMK_OUTPUT_FORMAT=text` for full verbose output. Coverage is `on` by default; override with `COVERAGE=off` if you explicitly need a no-coverage run.
