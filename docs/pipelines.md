...
- **One bmk provisions a venv at a time**  -  the provisioning is serialised across processes by an exclusive lock, so a subagent's `make test` beside your own, or `test-all` beside `test`, can no longer pull the environment out from under a running gate. Unguarded, that failure was silent: provisioning never raises, so the losing gate degraded to a different interpreter and type-checked, audited and tested the wrong environment with nothing in its output saying so. The lock covers the sync only, never a whole gate  -  a gate-lifetime lock would deadlock bmk against itself, since `push` runs the test pipeline and `test-all` spawns a cell per Python version inside one process tree. Its scope is one venv path, so the version matrix still provisions its cells in parallel. Lock files live in `BMK_VENV_LOCK_DIR` (default `~/.cache/bmk/venv-locks`), outside every repository, and `BMK_VENV_LOCK_TIMEOUT` (default 600s) bounds the wait; on timeout bmk changes nothing and says so on stderr. Editing `pyproject.toml` while a gate is already running is still unguarded.
- **bmk's own environment is not rebuilt under a running gate**  -  bmk installs itself once per machine, into uv's tool dir, and the Makefile upgrades that environment before every target. Every bmk holds a shared lock on it for its lifetime, and the upgrade takes the same lock exclusively, so a `make` in one repo waits instead of deleting site-packages out from under a gate running in another. Readers never exclude each other, so two repos still gate concurrently. The wait is bounded: an upgrade that cannot get the lock is skipped (the next `make` picks it up), while a repair that cannot get it fails rather than continuing on a damaged environment. `BMK_TOOL_LOCK=0` disables the guard. Two cases it cannot cover, by construction: a repo whose Makefile predates this is an unguarded writer until its next `make` regenerates it, and a hand-run `uv tool install bmk` is unguarded always - so when a command fails and the environment turns out to have been replaced mid-run, bmk says so instead of leaving an ImportError in its own dependencies to look like a flake.
